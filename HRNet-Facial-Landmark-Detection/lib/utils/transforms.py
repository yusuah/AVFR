...
def get_affine_transform(
    ...
    if isinstance(scale, (int, float)):
        scale_tmp = np.array([scale, scale], dtype=np.float32)
    else:
...
def transform_pixel(pt, center, scale, output_size, invert=0, rot=0):
    t = get_affine_transform(center, scale, rot, output_size, inv=invert)
    src_pt = np.array([pt[0] - 1, pt[1] - 1, 1.]).T
    new_pt = np.dot(t, src_pt)
    return new_pt[:2] 

def transform_preds(coords, center, scale, output_size):
    preds = coords.copy()
    if coords.shape[0] > 0:
        preds[0, 0:2] = transform_pixel(coords[0, 0:2], center, scale, output_size, 1, 0)
    
    for p in range(1, coords.shape[0]):
        preds[p, 0:2] = transform_pixel(coords[p, 0:2], center, scale, output_size, 1, 0)
