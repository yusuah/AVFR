...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.head = nn.Sequential(
            nn.Conv2d(
                in_channels=final_inp_channels, #입력: feature_map (shape: [B, final_inp_channels, H, W])
            ...
            nn.Conv2d(
                ...
                out_channels=config.MODEL.NUM_JOINTS, #출력: heatmap (shape: [B, NUM_JOINTS, H, W])
    ...
    def forward(self, x, return_feature_map=False):
        ...
        feature_map = torch.cat([x[0], x1, x2, x3], 1) #featuremap shape: (B, C_total, H, W)
        if return_feature_map:
            return feature_map
        x = self.head(feature_map)#scoremap shape: (B, num_kp, H, W)
